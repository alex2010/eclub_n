...
#        url.substring 0, url.length - 1
    ...
    menuLink: (it)->
        if !it.href
            if it.param
                p = if it.param.charAt(0) == '_' then eval(it.param.substr(1)) else it.param
            if it.entity
                it.href = cf.linkPre + it.entity + if it.action then '/' + it.action else ''
                it.href += if p then '/' + p else ''
            else if it.action
                if it.className
                    it.className += ' ' + it.action
                else
                    it.className = it.action + ' '
            else
                it.href = '#'
        id = 'm-' + it.id if it.id
        sp = if it.icon then '<i class="' + it.icon + '"></i>' else ''
        it.title = iin(it.label) unless it.title
        _vh.link((if it.href is 'x' then null else it.href), sp + it.title, it.description, it.className, id)
...
#                    position: 'absolute'
    ...
    arrangeFooter: ()->
        u = $('#footer .menus').first()
        h = $('h3', u)
        if h.length
            c = $('<div class="footerBox"/>')
            h.each(->
                cls = $(@).attr('class')
                if (uu = $('.' + cls, u)) && uu.length
                    c.append $('<ul class="sep"/>').append(uu)
            )
            u.remove()
            $('#footer .col-md-8').prepend c
        else
            u.after $('<ul/>').append($('.c4', u))
            u.after $('<ul/>').append($('.c3', u))
            u.after $('<ul/>').append($('.c2', u))
            u.after $('<ul class="fcx"/>').append($('.cx', u))
            $('#footer ul').show()

    sendEntityWeibo: (e, it, url)->
        opt =
            default_text: "#{_i.nav[e]}:#{it.title} #{cf._uri(e, it.id, url)} - #{cf.community.name}"
            default_image: @resPath(util.pImg(it))
        snc.sina.sendWidget(opt)

    sendCommentWeibo: (note, href)->
        if note.length > 100
            note = note.substr(0, 100)
        opt =
            default_text: "#{note} #{href} - #{cf.community.name}"
        snc.sina.sendWidget(opt)
    ...
    pStr: (it, p)->
        if it and _.isString(it[p]) and it[p].length > 1
            it[p] = JSON.parse(it[p])
        else if it[p] is null
            it[p] = {}
        else
            it[p]
...
#    initMapDlg: (mo, inputId, callback) ->
#        $(inputId).click (evt) ->
#            evt.preventDefault()
#            opt =
#                className: 'showMap'
#                title: mo.title || 'Select your location'
#                width: 650
#                data: mo.data || {}
#                shown: ->
#                    $(this).css "width", "100%"
#                    cf._dMap = new MapObj(mo).render()
#                    callback(cf._dMap) if callback
#                hidden: ->
#                    W.del('_dMap', cf)
#            mo.mapDlg = '#map-dlg'
#            mo.mapElem = '#dmap'
...
#            opt.width = mo.width
#            opt.getContent = ->
#                '<div id="map-dlg"><div id="dmap"></div></div>'
#            new _views.BlkView(opt).dlg()
#        return

#    showMapBlk: (mo, venue, renderId) ->
#        return  if window["google"] is `undefined` or not window["google"]?
#        $(renderId).append $("<h3/>").text(ii('nav.venue'))  if mo.showTitle
#        $(renderId).append(rTmpl("#mapTmpl")).append($("<div/>").addClass("textile").text(venue.name + ", " + venue.address + ", " + venue.telephone)).find("#map-show").addClass "map-show-frame"
#        c = "<a class=\"icon-search\" title=\"search information in google\" target=\"-_blank\" href=\"http://www.google.com.hk/?q=" + venue.name + "\">" + venue.name + "</a><br/>" + "<a class=\"icon-globe\" title=\"search route in google map\" target=\"-_blank\" href=\"" + mlu.paramsUrl("http://ditu.google.com/maps?",
#            f: "d"
#            source: "s_q"
#            q: venue.name
#            daddr: venue.name
...
#            iwloc: "A"
#            hl: getLanguage()
#        ) + "\">" + venue.address + "</a><br/>" + "<a class=\"icon-bell\" href=\"#\">" + venue.telephone + "</a>"
#        mapObj = new MapObj(mo)
#        mapObj.render()
#        mapObj.setZoom mo.mapOpt.zoom
#        mapObj.addMarker venue.lat, venue.lng, mapObj.map, c, null, null, true

#    geoShortName: (results, attr, isLong) ->
#        return unless google?
#        i = results[0].address_components.length - 1
#        while i >= 0
#            item = results[0].address_components[i]
#            return (if isLong then item.long_name else item.short_name)  if item.types.has(attr)
#            i--
#        null
#
#    getLatLng: (_lat, _lng) ->
#        return unless google?
#        if _lat and _lng
#            new google.maps.LatLng(parseFloat(_lat), parseFloat(_lng))
#        else
#            new google.maps.LatLng(parseFloat(lat), parseFloat(lng))
#
#    codeLatLng: (latlng, callback, noFound, error) ->
#        return unless google?
...
#            log "Sorry,get Geo location Error!"
#        getPositionSuccess = (position) ->
#            lat = position.coords.latitude
#            lng = position.coords.longitude
#            mlu.codeLatLng new google.maps.LatLng(lat, lng), (results) ->
#                curCity = mlu.geoShortName(results, "locality")
#                curCountry = mlu.geoShortName(results, "country", true)
#                if $("#top-panel ul li.greeting #geoMsg").length is 0
#                    $("<span id=\"geoMsg\">[" + curCity + "]</span>").appendTo "#top-panel ul li.greeting"
#                else
#                    $("geoMsg").text curCity
#            cf.lat = lat
#            cf.lng = lng
...
#                    timeout: 3000
#            else
#                log "Sorry,your device not support Geo location!"
#                getPositionError null
    ...
    actUrl: (entity, action, param) ->
        ...
        if param
            url += '/' + param
        url
#        + "/" + (if action then action else entity) + (if param then "/" + param else "")
...
#        inp
    ...
    sortTbPanel: (n)->
        opt =
            ...
            desc: 'desc'
        for k,v of opt
            "<a name='#{n}' class='sort' value='#{v}'>#{iin(k)}</a>"

    countDown: (start, duration, now = util.now())->
        if now > start
            if now < start + duration
                t: 'in'
                s: duration - (now - start)
            else
                t: 'after'
                s: now - (start + duration)
        else
            t: 'pre'
            s: start - now

    secondText: (l)->
        ld = parseInt(l / cf.dayTime)
        lh = parseInt((l % cf.dayTime) / cf.hourTime)
        lm = parseInt((l % cf.hourTime) / cf.minuteTime)
        ls = parseInt((l % cf.minuteTime) / 1000)
        "#{ld}天#{lh}小时#{lm}分种#{ls}秒"

    leftTimeText: (res)->
        if res.t is 'in'
            util.secondText res.s
        else if res.t is 'pre'
            '未开售'
        else
            '已销售完'
